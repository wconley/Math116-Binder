...
def vectorize(f, inputs, outputs):
    ...
    try:
        ...
        if outputs == 1 and isinstance(y, np.ndarray) and y.shape == (3,):
            return f
        if 1 < outputs == len(y) and all(
                isinstance(y0, np.ndarray) and y0.shape == (3,) for y0 in y):
            return f
